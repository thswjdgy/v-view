...
### 1.0 End-to-End AI 도구 체인 (기획 → 디자인 → 개발 → 검토)

v-view는 단계별로 **각 분야에 특화된 AI 도구**를 조합하고, 모든 산출물은
**개발자(손정협)가 직접 검토·결정**하는 Human-in-the-Loop 방식으로 진행했습니다.

```
[기획]          [디자인]            [개발]              [검토]
Manifest AI  →  Google Stitch  →  Claude Code   →   손정협 (개발자)
앱 기획·         UI/화면 디자인       코드 생성·          전 산출물 검수·
요구사항 도출      시안 생성           리팩토링·테스트       의사결정·방향수정
```

| 단계 | 사용 도구 | 산출물 | 검토 주체 |
|---|---|---|---|
| **기획(Plan)** | **Manifest AI** | 앱 컨셉·기능 정의·요구사항 초안 | 손정협 |
| **디자인(Design)** | **Google Stitch** | UI 화면 시안·디자인 시스템 | 손정협 |
| **개발(Develop)** | **Claude Code** | Flutter 코드·테스트·문서 | 손정협 |
| **검토(Review)** | 손정협 (개발자) | 최종 의사결정·품질 검수 | — |

> **핵심**: AI는 각 단계의 **생성(generation)**을 담당하고,
> **판단·검증·결정은 사람**이 한다. AI 산출물을 그대로 쓰지 않고
> 개발자가 전 과정을 검토해 방향을 수정했다. (검토 사례: [WIKI.md](WIKI.md) §5)

### 1.1 채택 Agent 구성 (개발 단계)
