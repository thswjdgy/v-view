# ADR-0003: 백엔드/비전 라이브러리 선택 — ML Kit + Claude API
...
- AI 질문/피드백: **Claude API** (Anthropic)
...
| 서비스 | 장점 | 제외 이유 |
|--------|------|-----------|
| **Claude API** | 한국어 품질 우수, 구조화 응답 | 선택 |
...
- Claude API: 별도 서버 없이 REST 호출만으로 AI 기능 구현
- Hive: Flutter 전용 경량 로컬 DB, 설정 단순

**리스크 및 대응**
- ML Kit 정확도 편차 → 얼굴 미검출 시 '측정 불가/참고용' 표시
- Claude API 비용 → 세션당 호출 횟수 제한으로 관리
...
> "시선 분석은 ML Kit, AI 질문 생성은 Claude API를 선택했습니다. ML Kit은 분석 모델이 앱 안에 내장되어 원본 영상을 서버로 보내지 않아 개인정보 정책을 충족하고, Claude API는 별도 서버 없이 REST 호출만으로 한국어 면접 질문 생성이 가능하기 때문입니다. MediaPipe는 정확도가 높지만 Flutter 연동 복잡도가 높아 7주 일정에 리스크가 컸습니다."
